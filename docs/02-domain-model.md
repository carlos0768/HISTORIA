...
### 6.1 歴史総合のうち日本史分野を範囲外にする（2026-09-04 追加・作者判断）

**§6 の「歴史総合を対象に含める」は保つ。外すのはその中の日本史分野だけである。**

理由（作者）: **世界史では出ないから。** 私大の世界史で版籍奉還や55年体制が問われることはない。
1本あたり約50円・約3分かかる教材を、出ない範囲に払う理由が無い。

**外し方は「行を消さない」。** `kc.retired = true` を立てる（`seed/kc.csv` の `retired` 列に
理由を書くと立つ）。行を消すと `item_kc` / `response` / `kc_region` の外部キーが道連れになり、
**一度でも解いた記録まで消える**。`retired` なら出題・教材・診断から外れるだけで、
気が変わったら列を空にすれば戻る。

**世界とつながる日本は残し、国内で完結する日本を外す。** これが判断の軸である。

| 外した節（KC が全部消えて、教材を作らなくなる） | なぜ |
|---|---|
| `gh.2.3.1` 開国と幕末の動乱 | 和親条約と修好通商条約の違い・尊王攘夷・開港場は日本史 |
| `gh.2.3.2` 明治維新と近代国家の形成 | 版籍奉還・地租改正・士族反乱・条約改正 |
| `gh.2.4.1` 大日本帝国憲法と初期議会 | 天皇大権・教育勅語・初期議会の争点 |
| `gh.3.2.1` 政党政治の展開と社会運動 | 大正デモクラシー・普通選挙法・治安維持法・関東大震災 |
| `gh.3.3.1` 世界恐慌の影響 | 題は世界だが中身は昭和恐慌・農村の窮乏・高橋財政。世界恐慌そのものは `wh.4.6.1` にある |
| `gh.4.1.1` 占領改革と独立の回復 | 三大改革・日本国憲法・独立の回復 |
| `gh.4.1.2` 高度経済成長と国際社会への復帰 | 高度成長・公害・太平洋ベルト |
| `gh.4.2.1` 冷戦の終結と日本 | バブル経済・55年体制 |
| `gh.4.2.2` 現代日本の課題 | 少子高齢化・災害と原子力 |

| 残した節 | なぜ残すか |
|---|---|
| `gh.2.4.2` 日清・日露戦争と国際関係 | **帝国主義の中核**。下関条約・三国干渉・韓国併合は世界史で問われる |
| `gh.3.3.2` 満洲事変と国際的孤立 | 満洲国・国際連盟の無力は世界史。国内政治への波及（五・一五事件）だけ外した |
| `gh.3.4.1` 日中戦争の長期化 | 援蒋ルート・対米関係の悪化は世界史。国家総動員体制だけ外した |
| `gh.3.4.2` アジア太平洋戦争と戦後処理 | 大東亜共栄圏・終戦・東アジアの分断は世界史。戦時下の国民生活だけ外した |
| `gh.3.1.1` 総力戦と国際秩序の変化 | 二十一か条・民族自決・委任統治は世界史。大戦景気だけ外した |
| `gh.2.1.1` / `gh.2.2.2` / `gh.3.1.2` | 18世紀アジア・国民国家・大衆社会。日本を扱う KC を1件ずつ外した |

**結果: KC 408件のうち 52件を範囲外にし、残り 356件。教材を作る節は 75 → 66。**
生成の見込みは 約 ¥3,900 → **約 ¥3,400**（`08` §3.4）。

★ **`retired` を見ていない経路があると、外したことにならない。**
`kcsForUnits`（特訓）・`unitTree`（範囲選択）・`redact.ts`（教材のプロンプト）・
診断（`KC_CELL_SQL`）はいずれも `NOT retired` を見ていたが、
**一括生成の対象一覧（`pendingUnits`）だけが見ていなかった**。そのまま流せば、
外したはずの9節へ約 ¥470 を払っていた。同時に直した。

★ **範囲内の KC が、範囲外にした KC を前提にしてはならない。** 前提が永久に出題されないと、
その KC は永久に「前提未習」で止まる。`prereq_ids` はまだ読まれていないので壊れても
画面には何も出ない。`seed/validate.mjs` の 8b で落とす。
（実際に2件あった: `kc.masssoc.media_power` と `kc.manchuria.order`。前提を外した）

★ **正典（`canon_event`）と人物（`person`）は減らさない。** あれは層2の照合材料であって
出題範囲ではない。日本の事件が入っていても、教材に日本の記述が出たときに照合できるだけで、
害が無い。減らすと検証が弱くなる。
