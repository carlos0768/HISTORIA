...
**ノルマの単位は問数であり、時間ではない**（作者判断 Q9・`14-open-questions.md` §5.1）。
1日の分数を持たせて教材の読解時間を含めて組み直す案は検討したうえで**採らない**。
上限を可変にしてある（§9.1）ので、多すぎると感じた利用者は自分で下げられる。
...
daily_queue(user, today) =
    全 active drill の KC の union に属する kc_card のうち
      (a) due_at <= today                … 復習     → ノルマの外。全件出す
      (b) kc_card が存在しない KC        … 新規学習 → §3.1 の target の範囲で投入する
    を priority 降順で並べ、合わせて MAX_DAILY 件で打ち切る。
    各 KC について、実際に出す設問は 04b-spaced-repetition.md §5 の pick_item() で決める。
```

### 5.1.1 復習と新規で扱いを分ける理由

**復習は §3.1 のノルマの外に置く。** `target` は「締切に間に合わせるための投入ペース」であり、
「今日思い出さないと忘れるカードの数」ではない。両者を同じ枠で絞ると次が起きる。

- 締切に余裕がある期間は `target` が小さくなる。極端には
  `reps_left` が全KCで 0 を返して `target = 0` になる日がある
- そのとき due のカードまで 0 件に絞られ、**間隔反復が止まる**。
  SM-2 が「今日が復習日だ」と決めた日に何も出さないのでは、`04b` の前提が崩れる

**新規学習は逆に `target` で絞る。** 絞らないと初日に全KCが投入され、
翌日から due の山が崩れなくなる（§4.2 が防ごうとしている状態を自分で作ることになる）。

合計は `MAX_DAILY` で打ち切る。復習だけで上限に達したらその日は新規を入れない。
**「今日やること」に表示する数字は、この合成後のキューの件数**である。

**今日すでに解いた KC の数を、新規の投入枠と上限の両方から差し引く。**
引かないと、解いた KC がキューから消えるたびに新しい KC が補充され、
**「今日やること」が 0 に到達しない**。1日の終わりが定義できず、
「今日の分は終わりました」を出す条件が存在しなくなる。
