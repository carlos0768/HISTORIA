...
| V2 | ~~候補収集は検索ではなく **チャンネル許可リスト方式**にする~~ **2026-09-05 撤廃**（作者判断）。§4.1.0 を見ること |
...
### 4.1.0 チャンネルの事前許可制は撤廃した（2026-09-05・作者判断）

**V2 を取り下げる。** 動画を1本も出せないまま止まっていた原因が、
「まずチャンネルを承認する」という最初の関門だったためである。
承認する人が動かなければ、その先の取り込みも対応付けも一歩も進まない。

撤廃したのは**関門だけ**である。次の3つはそのまま残す。

| 残すもの | 理由 |
|---|---|
| `channel_allowlist` 表 | 画面が局名を出すのに `video.channel_id` の参照先が要る（`lib/loop/video.ts` の `videosForKcs` が JOIN している）。役割を「事前に通す一覧」から**動画から自動で埋まる登録簿**へ変えるだけで、表は消さない |
| V4（動画1本ごとの承認） | 誰の作った何を配るかは、依然として作者が決める |
| V5（埋め込み不可・年齢制限を弾く） | 安全側の制約であって、速度の問題ではない |

`scripts/db/seed.ts` は、登録簿に無いチャンネルの動画が来たら**弾かずに登録する**。
ただし `channel_title` の無い行は通さない。画面は局名を出すので、空のまま通すと
「どこの誰が作ったか分からない動画を再生させる」ことになる。

★ §4.1.2 の除外判断は生きている。**関門を外したことは、何を貼ってもよいという意味ではない。**
事実誤りが報告されているものを避ける根拠は許可リストではなく `08-ai-architecture.md` §5 にある。
