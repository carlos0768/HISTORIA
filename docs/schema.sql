...
-- 歴史地球儀。canon_event はファクトチェック正典なので混在させない。
-- Atlas は出典・信頼度・描画可能な地理要素を必須にする（seed/atlas）。
...
CREATE TABLE atlas_event (
  id              text PRIMARY KEY,
  canon_event_id  text REFERENCES canon_event(id) ON DELETE SET NULL,
  ...
  end_year        int CHECK (end_year IS NULL OR (end_year <> 0 AND end_year BETWEEN -10000 AND 2100)),
  end_date        jsonb,
  exam_weight     real NOT NULL DEFAULT 1 CHECK (exam_weight BETWEEN 0 AND 3),
...
CREATE INDEX atlas_event_year_idx ON atlas_event (start_year, end_year);
CREATE INDEX atlas_event_tags_idx ON atlas_event USING gin (tags);

CREATE TABLE atlas_event_source (
  event_id  text NOT NULL REFERENCES atlas_event(id) ON DELETE CASCADE,
  source_id text NOT NULL REFERENCES atlas_source(id),
  claims    text[] NOT NULL,
  PRIMARY KEY (event_id, source_id)
);

CREATE TABLE atlas_event_unit (
  event_id text NOT NULL REFERENCES atlas_event(id) ON DELETE CASCADE,
  unit_id  text NOT NULL REFERENCES syllabus_unit(id),
  PRIMARY KEY (event_id, unit_id)
);

CREATE TABLE atlas_event_kc (
  event_id text NOT NULL REFERENCES atlas_event(id) ON DELETE CASCADE,
  kc_id    text NOT NULL REFERENCES kc(id),
  PRIMARY KEY (event_id, kc_id)
);

CREATE TABLE atlas_story (
  id          text PRIMARY KEY,
  title       text NOT NULL,
  summary     text NOT NULL,
  unit_id     text NOT NULL REFERENCES syllabus_unit(id),
  kind        text NOT NULL CHECK (kind IN ('journey','diffusion','chronology','conflict','exchange')),
  hero_year   int NOT NULL CHECK (hero_year <> 0),
  exam_weight real NOT NULL DEFAULT 1 CHECK (exam_weight BETWEEN 0 AND 3)
);

CREATE TABLE atlas_story_step (
  id          text PRIMARY KEY,
  story_id    text NOT NULL REFERENCES atlas_story(id) ON DELETE CASCADE,
  event_id    text NOT NULL REFERENCES atlas_event(id),
  ord         smallint NOT NULL CHECK (ord BETWEEN 1 AND 12),
  title       text NOT NULL,
  narration   text NOT NULL,
  duration_ms int NOT NULL CHECK (duration_ms BETWEEN 1200 AND 20000),
  UNIQUE (story_id, ord)
);
CREATE INDEX atlas_story_step_event_idx ON atlas_story_step (event_id);
...
-- 2026-05-30以降に作成したSupabaseプロジェクトでは public の新規表が
-- Data APIへ自動公開されない。RLSと別に読み取り権限を明示する。
