...
**生成も検証も課金**なので、制約はどちらも金額である（2026-09-04 に無料枠をやめた）
...
| Supabase は ap-northeast-1（東京）にあり、Vercel の関数の既定地域は iad1（米国東部） | **関数を hnd1（東京）に置く**（`vercel.json` の `regions`）。DB と認証への 1 往復が 150ms 前後から数 ms になる。遷移のたびに DB へ数往復するので、地域が離れているとそれだけで 1 秒近く待たされる |
...
| `historia-pages-*` | `/material/*`・`/textbook` **のみ** | **ネットワーク優先**。個人の内容を含むので消去の対象 |
