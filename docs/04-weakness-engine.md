...
> **実装時の実測（2026-09-02）。** 既に起草した共有プール408問を数えたところ、
> **12セル全部がすでに埋まっていた**。目標の20問/セルに足りないのは4セルだけで、
> 不足は合計 **34問**である（西アジア・アフリカ／南アジア・東南アジア・内陸アジアの
> 近世以降が薄い）。したがって「240問を新規に作る」必要はない。
> `npx tsx scripts/db/diagnostic-coverage.ts` がこの数を出す。
> **診断は生成を1問も回さずに今日から動く。**

> **共有プールは二重に使われている。** `item.user_id IS NULL` は診断だけでなく、
> 日々の出題（`app/study/page.tsx`）の供給源でもある。診断で最大24問を消費するぶん、
> その日の新規出題がその分だけ減る。648問中24問なので影響は小さいが、
> プールを絞る変更を入れるときは両方に効くことを忘れないこと。
...
> **実装時に見つけた食い違い（2026-09-02）。** この2つの条件は両立しない。
> 24問を12セルに配ると1セル2問である。四択（g=0.25）1問がθについて持つ
> Fisher 情報量は最良でも約0.14しかなく、事後SDを0.35にするには
> **1セルあたり約50問**が要る。事前SDを0.36未満まで縮めれば届くが、
> それは「最初から目標を満たしている」＝12問ちょうどで必ず打ち切る、という意味になる。
> つまり SD 条件が意味を持つ事前分布は存在しない。
> 実装は両方そのまま入れ、事前SDは 1.0（＝何も知らない）に置いた。
> 結果として実際に効くのは**上限24問**のほうで、SD 条件はほぼ発火しない。
> `lib/domain/diagnostic.ts` の `itemsNeededPerCell()` がこの計算を返し、
> 試験がそれを見張っている。目標SDを緩めれば早期打ち切りが働く。
