...
### 6.1 Free で始める（作者判断・条件付き）

**決定: Supabase Free で始める。困ってから Pro（約3,750円/月）に上げる。**

| | Free | Pro |
|---|---|---|
| 月額 | 0円 | 約3,750円 |
| 非アクティブ時の一時停止 | **あり**（条件は**要検証**） | なし |
| 自動バックアップ | 弱い | あり |
| DBサイズ | 本アプリには十分（教材400ユニット×4KB＝1.6MB、`response` 数万行） | — |

Free で始める理由:

1. **Phase 1〜3（18週）は作者1人が毎日使う**ので非アクティブにならない。
   一時停止が問題になるのは、友人を招待する Phase 4 以降である
2. 一時停止はデータの消失ではなく、復帰できる（**要検証**）
3. バックアップの弱さは `12-nonfunctional.md` §5 の**週1回の `pg_dump`** で代替できる。
   `response` さえ守れば他は全て再生成できる（§2.2）
4. 月3,750円を先払いする理由がない

**ただし条件を2つ課す。**

- **(a)** Phase 1 週1 で「**何日の非アクティブで停止するか／復帰は手動か自動か**」を実測する
- **(b)** 週1回の `pg_dump` を **Phase 1 から**実施する（`12-nonfunctional.md` §5 の規定を前倒し）

**Pro に上げる判断基準**: 友人が週末しか使わず、
**月曜にアプリが止まっている事象が2回起きたら** Pro に上げる。
