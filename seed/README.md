...
**次は層2の正典マスタ（`canon_event.csv` / `person.csv`）である。**
こちらは起草中で、承認欄は全件空欄になっている。

| ファイル | 中身 | 承認 |
|---|---|---|
| `canon_event.csv` | 年号照合の正典。目標 約1,200件 | **一括承認**（作者判断 2026-09-02） |
| `person.csv` | 人名照合の正典。目標 約400件 | 同上 |

正典が空だと層2の機械照合が空回りし、事実確認を層3（課金する Anthropic）だけが
**全 claim について**担うことになる（`docs/14` M26）。
