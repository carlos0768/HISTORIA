...
export async function createTestDb(name: string): Promise<{ db: Sql; drop: () => Promise<void> }> {
  ...
  const admin = postgres(TEST_DB_URL, { prepare: false, max: 1 })
  ...
  url.pathname = `/${name}`
  const db = postgres(url.toString(), { prepare: false, max: 6 })
  ...
  return {
    ...
    drop: async () => {
      ...
      const a = postgres(TEST_DB_URL!, { prepare: false, max: 1 })
