 ...
 * 名前を確認できる。1:110m の国土で描けない極小国（マルタ・バーレーン・
 * シンガポール等）は、10m から拾った重心に点で示される（MICRO_PINS）。
 * したがってここには通常どおり書いてよい。
 */
...
const LEAF: Readonly<Record<number, { label: string; countries: readonly string[] }>> = {
  2:  { label: '西欧', countries: ['250','276','826','372','528','056','442','756','040'] },
  3:  { label: '南欧・地中海', countries: ['380','724','620','300','196','470'] },
  4:  { label: '東欧・ロシア', countries: ['643','616','804','112','203','703','348','642','100','688','191','070','008','498','440','428','233','705','499','807'] },
  ...
  12: { label: 'アラビア半島', countries: ['682','887','512','784','414','634','048'] },
  14: { label: 'エジプト・北アフリカ', countries: ['818','434','788','012','504','732'] },
  15: { label: 'サハラ以南アフリカ', countries: ['566','231','404','834','800','288','466','686','729','728','706','148','562','120','180','178','024','894','716','508','450','710','072','516','324','854','204','384','430','694','478','140','266','454','646','108','232','262','768','624','270','426','226'] },
  16: { label: '南アジア', countries: ['356','586','050','144','524','064'] },
  17: { label: '東南アジア', countries: ['764','704','116','418','104','458','360','608','096','626','702'] },
...
export function unknownCountryCodes(): string[] {
  ...
  return [...new Set(REGION_SHAPES.flatMap(r => r.countries))]
    .filter(c => !(c in COUNTRY_PATHS) && !pinned.has(c))
}
