/**
 * 世界地図の基図を作る
 *
 *   node scripts/map/build-basemap.mjs
 *
 * 出典: Natural Earth 1:110m（パブリックドメイン）。
 * npm の world-atlas が TopoJSON として再配布しているものを変換している。
 * Natural Earth は「使用許諾を求める必要がない」と明示しているため、
 * docs/10 §2 の第三者著作物のリスクに当たらない。
 *
 * ★ 生成物 lib/map/basemap.ts はリポジトリに入れる。
 *   実行時に外部から地図を取りに行かない。CSP を広げずに済み、
 *   配信先が落ちても地図が消えない。
 *   （docs/design/Litverse Map System は d3 + Leaflet で CDN から取る作りだが、
 *     proxy.ts の CSP は img-src / connect-src を 'self' に絞っているため、
 *     タイル地図の側は採らず SVG 側だけを採っている）
 *
 * ★ 国ごとのパスを出す。地域の強調を矩形ではなく国土の形で塗るため
 *   （.lv-map__land--highlight）。
 */
import { readFileSync, writeFileSync } from 'node:fs'
import { feature } from 'topojson-client'

const topo = JSON.parse(readFileSync('node_modules/world-atlas/countries-110m.json', 'utf8'))
const countries = feature(topo, topo.objects.countries)

// 正距円筒図法。緯度は南緯60度から北緯84度まで（南極を落とし、北端は残す）
const LON0 = -180, LON1 = 180
const LAT0 = 84, LAT1 = -60
const W = 360, H = 144

const x = lon => ((lon - LON0) / (LON1 - LON0)) * W
const y = lat => ((LAT0 - lat) / (LAT0 - LAT1)) * H
const r = n => Math.round(n * 10) / 10

// 模式図なので細部は要らない。送る量を減らすために間引く
const MIN_RING = 1.0   // これより小さい輪（小島）は落とす。図の単位＝おおよそ1度
const MIN_STEP = 0.5  // 直前の点からこれ未満しか動かない点は落とす

/** 1つの輪を SVG のパスにする。範囲外・小さすぎ・日付変更線またぎを処理する */
function ringPath(ring) {
  if (ring.every(([, lat]) => lat < LAT1)) return ''   // 南極大陸

  const raw = ring.map(([lon, lat]) => [x(lon), y(Math.max(LAT1, Math.min(LAT0, lat)))])
  const xs = raw.map(p => p[0]), ys = raw.map(p => p[1])
  if (Math.max(...xs) - Math.min(...xs) < MIN_RING && Math.max(...ys) - Math.min(...ys) < MIN_RING) {
    return ''
  }

  const kept = [raw[0]]
  for (const p of raw.slice(1)) {
    const q = kept[kept.length - 1]
    if (Math.hypot(p[0] - q[0], p[1] - q[1]) >= MIN_STEP) kept.push(p)
  }
  if (kept.length < 4) return ''

  // ★ 日付変更線をまたぐ輪（ロシア・フィジーなど）は、経度が +180 から -180 へ
  //   飛ぶときに地図を横断する線を引いてしまう。またいだ箇所でパスを切る。
  const parts = []
  let run = [kept[0]]
  for (const p of kept.slice(1)) {
    if (Math.abs(p[0] - run[run.length - 1][0]) > W / 2) { parts.push(run); run = [p] }
    else run.push(p)
  }
  parts.push(run)

  return parts
    .map(part => part.map(([px, py]) => `${r(px)},${r(py)}`).filter((q, i, a) => q !== a[i - 1]))
    .filter(part => part.length >= 4)
    .map(part => `M${part.join('L')}Z`)
    .join('')
}

function geometryPath(geom) {
  const polys = geom.type === 'Polygon' ? [geom.coordinates] : geom.coordinates
  return polys.flat().map(ringPath).join('')
}

const paths = {}
const names = {}
for (const f of countries.features) {
  const d = geometryPath(f.geometry)
  if (!d) continue
  paths[String(f.id)] = d
  names[String(f.id)] = f.properties.name
}

/** 経緯線。30度ごと。docs/design/litverse-map.css の .lv-map__graticule で描く */
const grat = []
for (let lon = LON0 + 30; lon < LON1; lon += 30) grat.push(`M${r(x(lon))},0L${r(x(lon))},${H}`)
for (let lat = 60; lat > LAT1; lat -= 30) grat.push(`M0,${r(y(lat))}L${W},${r(y(lat))}`)
// 赤道は他と区別できるよう最後に置く（利用側で分けたくなったときのため）
const EQUATOR = `M0,${r(y(0))}L${W},${r(y(0))}`

const total = Object.values(paths).reduce((n, d) => n + d.length, 0)

const out = `/**
 * 世界地図の基図（自動生成）
 *
 * ★ 手で編集しない。scripts/map/build-basemap.mjs が作る。
 *
 * 出典: Natural Earth 1:110m（パブリックドメイン）
 * 図法: 正距円筒図法。経度 ${LON0}〜${LON1}、緯度 ${LAT1}〜${LAT0}
 * 用途: 学習用の模式図であり、正確な国境や領域を示すものではない
 *
 * 1:110m には極小国（マルタ・バーレーン等）が含まれない。
 * 世界史の地域区分には影響しないため、そのまま落としている。
 */
export const MAP_VIEWBOX = '0 0 ${W} ${H}' as const
export const MAP_WIDTH = ${W}
export const MAP_HEIGHT = ${H}
export const MAP_LON = [${LON0}, ${LON1}] as const
export const MAP_LAT = [${LAT1}, ${LAT0}] as const

/** 経度・緯度を図の座標に移す */
export function project(lon: number, lat: number): { x: number; y: number } {
  return {
    x: ((lon - ${LON0}) / ${LON1 - LON0}) * ${W},
    y: ((${LAT0} - Math.max(${LAT1}, Math.min(${LAT0}, lat))) / ${LAT0 - LAT1}) * ${H},
  }
}

/** 経緯線（30度ごと） */
export const GRATICULE_PATH = '${grat.join('')}'
/** 赤道 */
export const EQUATOR_PATH = '${EQUATOR}'

/** ISO 3166-1 の数値コード → 国土のパス */
export const COUNTRY_PATHS: Readonly<Record<string, string>> = ${JSON.stringify(paths, null, 0)}

/** 数値コード → 英語名。地域の対応表を作るときの確認に使う */
export const COUNTRY_NAMES: Readonly<Record<string, string>> = ${JSON.stringify(names, null, 0)}
`
writeFileSync('lib/map/basemap.ts', out)
console.log(`国 ${Object.keys(paths).length} / パス合計 ${total} 文字 / 経緯線 ${grat.length} 本 → lib/map/basemap.ts`)
