...
| 「調べる」 | 語の一致＋pgvector の近傍検索で教材の節・KC・正典を引き、**年表（行＝地域）と地図**に置く（[`11`](./docs/11-ux.md) §4.1）。専用ページ `/research` と、教材の本文の語から。埋め込みが無くても語の一致で動く。国名を入れると**版図の移り変わりを再生**できる（20国家・暫定マスタ。オスマン帝国の 3 段階は historical-basemaps の実境界、[`10`](./docs/10-legal-risk.md) §7b） |
| AI 生成 | プロバイダ抽象層のみ。**鍵が無い間はフェイクで通る** |
| PWA | オフラインは**読むだけ**。出題は端末に降ろさない（[`12`](./docs/12-nonfunctional.md) §10） |
| 本文のリンク | 教材の本文に出てくる**人物と出来事**を正典（`person` 446件・`canon_event` 1,180件）と突き合わせ、その語から**日本語版 Wikipedia の記事**へ別タブで飛べる（[`11`](./docs/11-ux.md) §4.2）。同じ位置では最も長い語を採り、短い語は前後の文字種を見て語の途中では拾わない。1つの語は初出だけ |
| テスト | 1131件（`npm test`） |
...
`kc` / `canon_event` / `material_section` の `embedding` のうち **空の行だけ**を `EMBED_MODEL` で埋める。
