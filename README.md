...
| 本文のリンク | 教材の本文に出てくる**人物と出来事**を正典（`person` 446件・`canon_event` 1,180件）と突き合わせ、その語から「調べる」へ飛べる（[`11`](./docs/11-ux.md) §4.2）。同じ位置では最も長い語を採り、短い語は前後の文字種を見て語の途中では拾わない。1つの語は初出だけ |
| テスト | 1110件（`npm test`） |
